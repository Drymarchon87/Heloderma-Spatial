...
Graph1<-ggplot(year,aes(x=N100,y=Home_Range_100mcp,group=Environment))+
  ...
  geom_smooth(method=lm)+scale_colour_manual(values=c(subsidized="cyan3",nonsubsidized="indian      red1"))+
...
```
Figure 2 | Non-Subsidized (Owl Head Buttes) vs. Subsidized (Stone Canyon) population 100% MCPs by number of fixes across the whole study interval.
...
LSM.YearHR<-ggplot(ref_dfRM_sex, aes(x=Sex,y=lsmean,group=Environment))+
  geom_point(aes(shape = factor(Environment)), size = 2,position=position_dodge(.1))+
...
Figure 7 | Paiwise comparisons of sex between environments. If red arrows overlap those of others, then  there is no significant statistical difference. 
 ...
 At Stone Canyon, mean male Gila Monster home range was 11.7 ± 1.7 S.E. ha (100% MCP), 9.1 ± 1.3 S.E. Ha., with a mean female home range at 9.8 ± 1.1 S.E. ha (100% MCP), 7.1 ± 0.71 S.E. ha. Mean home ranges of male and females at Owl Head Buttes were 43.2 ± S.E. 5.8 ha (100% MCP), 42.0 ± 4.9 S.E. ha (95% MCP), and 22.1 ± 3.7 S.E. ha (100% MCP), 20.6 ± 2.6 S.E. ha (95% MCP) respectively. In the pooled data of both populations, there was no effect of year on the analyses (F = 0.50, P = 0.48), while a significant difference between the two populations was detected (F = 8.86, P = 0.003), and a significance in sex (F = 15.75, P = 0.0004). Post-Hoc analyses between sexes indicated that there was a significant difference in male home ranges between the two environments (df = 70.4, P = 0.0007).  However, the difference in female home ranges between the two environments was not significant (df = 68.9, P = 0.06). At stone canyon there was no significant difference between male and female home ranges (df = 36.9, P = 0.23) with male home ranges being only 3% larger than females, while males at Owl Head Buttes had significantly larger home ranges than did females (df = 31.3, P = 0.0003) with males having 65% larger home ranges than females. Interestingly, males at Stone Canyon had smaller home ranges than did the females at Owl Head Buttes (Table ____), suggesting that Gila Monster home ranges in a resource subsidized population is greatly influenced by environmental factors of both populations.  When using 95% MCPs, male home ranging reduced by 25% and female range by 31%. At Owl Head Buttes, Gila Monsters showed a similar behavior with male home ranges reduced by 20% and female ranges reduced by 26% using 95% MCPs. 
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
SEAS.TEST<-ggplot(SEAS_GRP_TEST,aes(x=Season,y=Home_Range_100mcp,group=Environment))+
  geom_point(aes(shape = factor(Sex)),size=2,position=pd_RM)+
  geom_errorbar(aes(ymin=Home_Range_100mcp-se, ymax=Home_Range_100mcp+se),position = pd_RM,
                width = 0.2)+
  ggtitle("Seasonal Means")+
  xlab("Season")+
  ylab("Area (ha) using 100% MCP")+
  theme(plot.title = element_text(hjust = 0.5, color="black", size=14, face="bold"))
...
# SEAS.TEST<-ggplot(SEAS_GRP_TEST,aes(x=Environment,y=Home_Range_100mcp,group=c("Season")))+geom_point(aes(shape = factor(Sex)), size=2,position=position_jitter(.2))+geom_errorbar(aes(ymin=Home_Range_100mcp-se, ymax=Home_Range_100mcp+se),position=position_dodge(),width=.1)+ggtitle("Raw Seasonal Means")+xlab("Season")+ylab("Area (ha) using 100% MCP")+theme(plot.title = element_text(hjust = 0.5, color="black", size=14, face="bold"))

# SEAS.TEST<-ggplot(SEAS_GRP_TEST,aes(x=Environment,y=Home_Range_100mcp,group=c("Season")))+
#   geom_point()+geom_errorbar(aes(ymin=Home_Range_100mcp-se,
#                               ymax=Home_Range_100mcp+se),position=position_dodge(),width=.1)+
#   ggtitle("Raw Seasonal Means")+
#   xlab("Season")+
#   ylab("Area (ha) using 100% MCP")+
#   theme(plot.title = element_text(hjust = 0.5, color="black", size=14, face="bold"))
# 
# SEAS.TEST + facet_grid(Sex~Season) 
...
# SEAS.TEST<-ggplot(SEAS_GRP_TEST, aes(x = Environment, y = Home_Range_100mcp, fill = Sex)) +
...
LSM.SeasHR<-ggplot(ref_dfRM_season,aes(x=Environment,y=lsmean,group=c("Season")))+
  geom_point(aes(shape = factor(Sex)), size=2,position=pd_RM) +
  geom_errorbar(aes(ymin=lsmean-SE, ymax=lsmean+SE),position=pd_RM,width=.1)+
  ggtitle("Adjusted Seasonal Means")+
  xlab("Environment")+
  ylab("Area (ha) using 100% MCP")+
  theme(plot.title = element_text(hjust = 0.5, color="black", size=14, face="bold"))
