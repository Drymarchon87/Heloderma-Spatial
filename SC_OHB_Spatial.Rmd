...
RMmod.year95<-lmer(Home_Range_95mcp~Environment+Year+Sex+N95+Environment*Sex+
                   (1|Gila),data = year)
refRM_sex95 <- lsmeans(RMmod.year95, specs = c("Environment","Sex"))
ref_dfRM_sex95 <- as.data.frame(summary(refRM_sex95))

kable(ref_dfRM_sex95, format = "pandoc", caption = 'Table 3 | Directional means of home range (95% MCP) after being adjusted for year, sex and sample size.')
```
...
SC_stamen_map<-SC_stamen_map + ggsn::scalebar(x.min = -110.972, x.max = -110.966,
                     ...
                     model = 'WGS84') 
SC_stamen_map
SC_stamen_map+north2(SC_stamen_map, x = 0.89, y = 0.83, scale = 0.1, symbol = 16)
```
...
kable(ref_dfRM_KDE, format = "pandoc", caption = 'Table  | Subsidized directional means of KDE home ranges after being adjusted for year, sex and sample size.')
```
